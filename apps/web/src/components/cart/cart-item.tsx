...
export function CartItem({ item }: CartItemProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 p-3 sm:p-4 border rounded-lg">
      <Link href={`/products/${item.product.slug}`} className="flex-shrink-0">
        <div className="relative w-full sm:w-20 md:w-24 h-40 sm:h-20 md:h-24 rounded bg-gray-100">
          <Image
            src={item.product.images[0] || '/placeholder.jpg'}
            alt={item.product.name}
            fill
      ...
      <div className="flex-1 flex flex-col gap-2">
        <div className="flex justify-between gap-2">
          ...
          >
            <h3 className="font-semibold hover:underline text-sm md:text-base truncate">
              {item.product.name}
            </h3>
          </Link>
